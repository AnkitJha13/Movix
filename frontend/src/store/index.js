...
const getRawData = async (api, genres, paging = false) => {
  const moviesArray = [];
  for (let i = 1; moviesArray.length < 60 && i < 15; i++) {
    const {
      data: { results },
...
export const getUsersLikedMovies = createAsyncThunk(
  ...
  async (email) => {
    const {
      data: { movies },
    } = await axios.get(`https://movix-ulwf.onrender.com/api/user/liked/${email}`);
    return movies;
  }
);
...
export const removeMovieFromLiked = createAsyncThunk(
  ...
  async ({ movieId, email }) => {
    const {
      data: { movies },
    } = await axios.put("https://movix-ulwf.onrender.com/api/user/remove", {
      email,
      movieId,
    });
    return movies;
